  ...
  <transition
    ...
    <aside v-show="$page.props.showingMobileMenu" class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-white md:hidden">
      <div class="py-4 text-gray-500 dark:text-gray-400">
        <Link class="ml-4 text-lg font-bold text-gray-800" :href="route('simulacro-inicio')">
        INICIO
      </Link>
        <ul class="mt-6">
          <li class="relative px-4 py-1">
            <ResponsiveNavLink :class="[route().current('simulacro-inicio')? 'activado':'']" :href="route('simulacro-inicio')">
              <template #icon>
          ...
          <li class="relative px-4 py-1">
            <ResponsiveNavLink :href="route('simulacro-inscritos')" :class="[route().current('simulacro-inscritos')? 'activado':'']">
              <template #icon>
          ...
          <li class="relative px-4 py-1">
            <ResponsiveNavLink :href="route('simulacro-pagos')" :class="[route().current('simulacro-pagos')? 'activado':'']">
              <template #icon>
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                     xmlns="http://www.w3.org/2000/svg">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
