...
class BlogController extends Controller
{
    ...
    public function getPuntajesProceso(Request $request){
        ...
        $res = DB::table('puntajes as pun')
        ->join('postulante as pos', 'pos.nro_doc', '=', 'pun.dni')
        ->join('inscripciones as ins', function ($join) {
            $join->on('ins.id_postulante', '=', 'pos.id')
                 ->where('ins.estado', '=', 0);
        })
        ->join('programa as pro', 'pro.id', '=', 'ins.id_programa')
        ->where('pun.dni', $request->dni)
        ->whereColumn('pun.id_proceso', 'ins.id_proceso')
        ->where('pun.id_proceso', $request->id_proceso)
        ->orderBy('pun.fecha', 'desc')
        ->select([
            'pun.fecha', 'pun.dni', 'pos.nombres', 'pos.primer_apellido', 'pos.segundo_apellido',
            'pun.modalidad', 'pun.puntaje', 'pun.puntaje_vocacional', 'pun.apto as condicion',
            'pun.programa', 'pro.nombre as programa'
        ])
        ->get();
